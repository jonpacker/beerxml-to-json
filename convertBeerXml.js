...
var convert = function(beerxml, emit, cb) {
  ...
  if (beerxml.RECIPES.RECIPE.length > 1) {
    emit('warning', 'There is more than one recipe in the input XML.')
    var discardedRecipes = beerxml.RECIPES.RECIPE.map(function(recipe, i) {
      if (i == 0) return;
      return recipe.NAME[0];
    });
    discardedRecipes.shift();
    var discarded = discardedRecipes.join(', ');
    emit('warning', 'The following recipes will be discarded: ' + discarded);
  }
